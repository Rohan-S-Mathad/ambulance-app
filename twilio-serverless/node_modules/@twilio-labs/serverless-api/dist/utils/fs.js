 ...
 * @returns
 */
async function fileExists(filePath, hasWriteAccess = false) {
    try {
        await (0, exports.access)(filePath, hasWriteAccess ? READ_WRITE : READ_ONLY);
        return true;
    }
    catch (err) {
        return false;
 ...
 */
async function getDirContent(dir, extension) {
    const rawFiles = (await (0, exports.readDir)(dir));
    const unfilteredFiles = await Promise.all(rawFiles.map(async (filePath) => {
        if (!path_1.default.isAbsolute(filePath)) {
            filePath = path_1.default.join(dir, filePath);
        }
        const entry = await (0, exports.stat)(filePath);
        if (!entry.isFile()) {
            return undefined;
        }
 ...
 */
async function getListOfFunctionsAndAssets(cwd, config = {}) {
    let functionsDir;
    try {
 ...
 */
async function getServerlessConfigs(dirContent, ignoreExtension) {
    return Promise.all(dirContent.map(async (file) => {
        ...
        const content = await (0, exports.readFile)(file.path, encoding);
        return {
            name: path,
            path,
            access,

...
async function fetchListOfTemplates() {
    const response = await (0, got_1.default)(exports.TEMPLATES_URL, { responseType: 'json' });
...
async function getNestedRepoContents(url, dirName, directory) {
    const response = await getFromUrl(url);
    const repoContents = response.body;
    return (0, lodash_flatten_1.default)(await Promise.all(repoContents.map(async (file) => {
        if (file.type === 'dir') {
            return await getNestedRepoContents(file.url, path_1.default.join(dirName, file.name), directory);
        }
        else {
            return {
                name: file.name,
...
async function getFiles(templateId, directory) {
    const response = await getFromUrl(exports.CONTENT_BASE_URL + `/${templateId}/${directory}?ref=${TEMPLATE_BASE_BRANCH}`);
    const repoContents = response.body;
    return (0, lodash_flatten_1.default)(await Promise.all(repoContents.map(async (file) => {
        if (file.type === 'dir') {
...
async function getTemplateFiles(templateId) {
    try {
        const headers = buildHeader();
        const response = await (0, got_1.default)(exports.CONTENT_BASE_URL + `/${templateId}?ref=${TEMPLATE_BASE_BRANCH}`, {
            responseType: 'json',
            headers,
        });
        const repoContents = response.body;
        ...
        const files = otherFiles.concat(...(await Promise.all([assets, functions])));
        return files;
    }
    catch (err) {

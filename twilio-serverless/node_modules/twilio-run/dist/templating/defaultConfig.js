...
async function writeDefaultConfigFile(baseDir, overrideExisting = false, fileName = '.twilioserverlessrc') {
    const fullConfigFilePath = path_1.default.resolve(baseDir, fileName);
    const configFileExists = await (0, fs_1.fileExists)(fullConfigFilePath, true);
    if (configFileExists && !overrideExisting) {
        return false;
    }
    const content = templateDefaultConfigFile();
    try {
        await (0, fs_1.writeFile)(fullConfigFilePath, content, 'utf8');
        return true;
    }
    catch (err) {

...
function printAsset(asset, config) {
    ...
    return (0, chalk_1.default) `{dim ${config.url}${prefix}}${asset.path}`;
}
function printFunction(fn, config) {
    return (0, chalk_1.default) `{dim ${config.url}}${fn.path}`;
}
...
function prettyPrintAsset(asset, config) {
    ...
    const link = (0, utils_1.terminalLink)(`${accessPrefix}${assetPath}`, pathAccess);
    return link;
}
function prettyPrintFunction(fn, config) {
    const accessPrefix = fn.access === 'protected' ? chalk_1.default.cyan.bold('[protected] ') : '';
    const link = (0, utils_1.terminalLink)(`${accessPrefix}${fn.path}`, config.url + fn.path);
    return link;
}
function printPrettyRouteInfo(functions, assets, config) {
    const functionHeading = (0, chalk_1.default) `{green.bold Twilio functions available:}`;
    let functionInfo;
    if (functions.length > 0) {
        ...
        functionInfo = (0, chalk_1.default) `  {yellow ${log_symbols_1.default.warning}} No functions found`;
    }
    const assetHeading = (0, chalk_1.default) `{green.bold Twilio assets available:}`;
    let assetInfo;
    if (assets.length > 0) {
        assetInfo = assets
            .map((asset, idx) => {
        ...
        assetInfo = (0, chalk_1.default) `  {yellow ${log_symbols_1.default.warning}} No assets found`;
    }
    let ngrokInfo = '';
    if (config.url.includes('ngrok.io')) {
        ngrokInfo = (0, chalk_1.default) `{green.bold ngrok request inspector available:}\nhttp://127.0.0.1:4040`;
    }
    const output = [
        functionHeading,
        functionInfo,
        ...
        .join('\n')
        .trim();
    const wrappedOutput = (0, wrap_ansi_1.default)(output, utils_1.windowSize.width - 8, {
        wordWrap: true,
    });
    return (0, boxen_1.default)(wrappedOutput, { padding: 1 });
}
...
async function printRouteInfo(config) {
    ...
    const { functions, assets } = await (0, runtime_paths_1.getFunctionsAndAssets)(config.baseDir, searchConfig);
    let output;

...
exports.printListResult = void 0;
...
const headingTransform = (name) => {
    return chalk_1.default.cyan.bold((0, lodash_startcase_1.default)(name).replace(/Sid$/g, 'SID'));
};
function printRows(rows, keys) {
    return (0, columnify_1.default)(rows, { columns: keys, headingTransform });
}
...
function printListResultPlain(result, config) {
    ...
    if (types.length === 1) {
        (0, output_1.writeOutput)(printSection(types[0], result[types[0]], config));
        return;
    }
    for (const type of types) {
        const section = result[type];
        const output = printSection(type, section, config);
        (0, output_1.writeOutput)((0, lodash_startcase_1.default)(type) + '\n' + output + '\n');
    }
}
function prettyPrintFunctionsOrAssets(result) {
    ...
    const resourceString = entries
        ...
        const suffix = entry.visibility === 'public'
            ? ' '
            : (0, chalk_1.default) `{dim [Visibility {reset.bold ${entry.visibility}}]}`;
        return (0, common_tags_1.stripIndent)((0, chalk_1.default) `
...
function prettyPrintVariables(variables) {
    ...
    const updatedRows = entries.map((entry) => {
        return {
            ...entry,
            key: (0, chalk_1.default) `{dim │} {cyan ${entry.key}}:`,
        };
    });
    const renderedValues = (0, columnify_1.default)(updatedRows, {
        columns: ['key', 'value'],
        showHeaders: false,
    });
...
function prettyPrintSection(sectionTitle, sectionContent) {
    let sectionHeader = chalk_1.default.cyan.bold(`${(0, title_1.default)(sectionTitle)}:`);
    let content = '';
    if (sectionTitle === 'builds') {
        ...
        sectionHeader = (0, chalk_1.default) `{cyan.bold ${(0, title_1.default)(sectionTitle)}} {dim for environment ${data.environmentSid}}`;
    ...
    const output = (0, common_tags_1.stripIndent) `
    ${sectionHeader}\n\n${content}\n\n\n
  `;
    return output;
}
function printListResultTerminal(result, config) {
    ...
    let metaInfo = (0, common_tags_1.stripIndent)((0, chalk_1.default) `
    {cyan.bold Username}     ${config.username}
    {cyan.bold Password}     ${(0, utils_1.redactPartOfString)(config.password)}
  `);
    if (config.serviceSid || config.serviceName) {
        metaInfo += (0, chalk_1.default) `\n{cyan.bold Service}      ${config.serviceSid || config.serviceName}`;
    }
    if (config.environment) {
        metaInfo += (0, chalk_1.default) `\n{cyan.bold Environment}  ${config.environment}`;
    }
    logger_1.logger.info(metaInfo + '\n');
    (0, output_1.writeOutput)(output);
}
function printListResult(result, config, outputFormat) {
    if (outputFormat === 'json') {
        (0, output_1.writeJSONOutput)(result);
        return;
    }

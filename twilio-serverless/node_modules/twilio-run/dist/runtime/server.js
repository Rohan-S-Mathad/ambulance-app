...
async function findRoutes(config) {
    const searchConfig = {};
...
async function createLocalDevelopmentServer(port = DEFAULT_PORT, config) {
    try {
        ...
        server.on('updated-routes', async (config) => {
            await (0, start_1.printRouteInfo)(config);
        });
...
async function createServer(port = DEFAULT_PORT, config) {
    ...
    let routeMap = await (0, route_cache_1.getRouteMap)(config);
    if (config.live) {
        const watcher = chokidar_1.default.watch([
        ...
        const reloadRoutes = async () => {
            routeMap = await (0, route_cache_1.getRouteMap)(config);
            await (0, start_1.printRouteInfo)(config);
        };
        // Debounce so we don't needlessly reload when multiple files are changed
        const debouncedReloadRoutes = (0, lodash_debounce_1.default)(reloadRoutes, RELOAD_DEBOUNCE_MS);
...
async function runServer(port = DEFAULT_PORT, config) {

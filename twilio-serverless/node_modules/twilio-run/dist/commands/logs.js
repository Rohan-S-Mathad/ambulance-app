...
async function handler(flags, externalCliOptions) {
    ...
    let config;
    try {
        config = await (0, logs_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
        logError(err.message);
    ...
    try {
        ...
        if (flags.tail) {
            ...
            const result = (await client.getLogs({ ...config }));
...
exports.cliInfo = {
    options: {
        ...
        production: {
            ...
            describe: 'Retrieve logs for the production environment. Overrides the "environment" flag',
        },
    },
};

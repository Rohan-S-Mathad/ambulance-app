...
async function handler(flags, externalCliOptions) {
    ...
    flags.cwd = cwd;
    const continueWork = await (0, legacy_config_1.default)(cwd);
    if (!continueWork) {
        process.exit(1);
    }
    const command = (0, utils_2.getFullCommand)(flags);
    await (0, project_structure_1.default)(cwd, command, true);
    let config;
    try {
        config = await (0, deploy_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
    ...
    try {
        ...
        await (0, utils_1.saveLatestDeploymentData)(config.cwd, serviceSid, buildSid, config.username.startsWith('AC')
            ? config.username
...
exports.cliInfo = {
    options: {
        ...
        production: {
            ...flags_1.ALL_FLAGS['production'],
            describe: 'Please prefer the "activate" command! Deploys to the production environment (no domain suffix). Overrides the value passed via the environment flag.',
            default: false,
        },
    },
};

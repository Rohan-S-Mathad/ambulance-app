"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.builder = exports.describe = exports.command = exports.cliInfo = exports.handler = void 0;
const chalk_1 = __importDefault(require("chalk"));
const inquirer_1 = __importDefault(require("inquirer"));
const path_1 = __importDefault(require("path"));
const project_structure_1 = __importDefault(require("../checks/project-structure"));
const flags_1 = require("../flags");
const actions_1 = require("../templating/actions");
const logger_1 = require("../utils/logger");
const utils_1 = require("./utils");
async function getMissingInfo(flags) {
    const questions = [];
    if (!flags.template) {
        const templates = await (0, actions_1.fetchListOfTemplates)();
        const choices = templates.map(template => {
            return {
                name: (0, chalk_1.default) `${template.name} - {dim ${template.description}}`,
                value: template.id,
            };
        });
        questions.push({
            type: 'list',
            name: 'template',
            message: 'Select a template',
            choices,
        });
    }
    if (!flags.namespace) {
        questions.push({
            type: 'input',
            name: 'namespace',
            message: 'What should be the namespace your function(s) are placed under?',
            validate: (input) => {
                if (input.length < 1 || input.includes(' ')) {
                    return 'Your name cannot include whitespace';
                }
                return true;
            },
        });
    }
    if (questions.length === 0) {
        return {
            ...flags,
            namespace: flags.namespace,
            template: flags.template,
        };
    }
    const answers = await inquirer_1.default.prompt(questions);
    return {
        ...flags,
        template: flags.template || answers.template,
        namespace: flags.namespace || answers.namespace,
    };
}
function getBaseDirectoryPath() {
    const currentDir = process.cwd();
    const baseName = path_1.default.basename(currentDir);
    if (baseName === 'functions' ||
        baseName === 'assets' ||
        baseName === 'src' ||
        baseName === 'static' ||
        baseName === 'readmes') {
        return path_1.default.resolve(currentDir, '..');
    }
    return currentDir;
}
async function handler(flagsInput, externalCliOptions) {
    (0, logger_1.setLogLevelByName)(flagsInput.logLevel);
    const targetDirectory = getBaseDirectoryPath();
    const command = (0, utils_1.getFullCommand)(flagsInput);
    await (0, project_structure_1.default)(targetDirectory, command, true);
    const flags = await getMissingInfo(flagsInput);
    if (typeof flags.namespace === 'undefined' ||
        flags.namespace.length === 0 ||
        typeof flags.template === 'undefined' ||
        flags.namespace.length === 0) {
        return;
    }
    const sanitizedNamespace = flags.namespace.replace(/\.js$/, '');
    try {
        await (0, actions_1.downloadTemplate)(flags.template, sanitizedNamespace, targetDirectory);
    }
    catch (error) {
        logger_1.logger.error(error.message, error.name);
    }
}
exports.handler = handler;
exports.cliInfo = {
    options: {
        ...(0, flags_1.getRelevantFlags)([...flags_1.BASE_CLI_FLAG_NAMES, 'template']),
    },
};
function optionBuilder(yargs) {
    yargs = yargs.example('$0 new hello-world --template=blank', 'Creates a basic blank template as hello-world function');
    yargs = Object.keys(exports.cliInfo.options).reduce((yargs, name) => {
        return yargs.option(name, exports.cliInfo.options[name]);
    }, yargs);
    return yargs;
}
exports.command = ['new [namespace]', 'template [namespace]'];
exports.describe = 'Creates a new Twilio Function based on an existing template';
exports.builder = optionBuilder;

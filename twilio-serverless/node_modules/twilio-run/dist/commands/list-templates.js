"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.builder = exports.describe = exports.command = exports.cliInfo = exports.handler = void 0;
const chalk_1 = __importDefault(require("chalk"));
const flags_1 = require("../flags");
const actions_1 = require("../templating/actions");
const logger_1 = require("../utils/logger");
const output_1 = require("../utils/output");
async function handler(flags) {
    (0, logger_1.setLogLevelByName)(flags.logLevel);
    const spinner = (0, logger_1.getOraSpinner)('Fetching available templates').start();
    let templates;
    try {
        templates = await (0, actions_1.fetchListOfTemplates)();
    }
    catch (err) {
        spinner.fail('Failed to retrieve templates');
        process.exitCode = 1;
        return;
    }
    spinner.stop();
    if (flags.outputFormat === 'json') {
        (0, output_1.writeJSONOutput)(templates);
    }
    else {
        templates.forEach((template) => {
            (0, output_1.writeOutput)((0, chalk_1.default) `‣ ${template.name} ({cyan ${template.id}})\n  {dim ${template.description}}`);
        });
    }
}
exports.handler = handler;
exports.cliInfo = {
    options: { ...(0, flags_1.getRelevantFlags)(['output-format']) },
};
function optionBuilder(yargs) {
    yargs = Object.keys(exports.cliInfo.options).reduce((yargs, name) => {
        return yargs.option(name, exports.cliInfo.options[name]);
    }, yargs);
    return yargs;
}
exports.command = ['list-templates'];
exports.describe = 'Lists the available Twilio Function templates';
exports.builder = optionBuilder;

...
const env_import_1 = require("../../config/env/env-import");
...
async function handler(flags, externalCliOptions) {
    ...
    let config;
    try {
        config = await (0, env_import_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
        logger_1.logger.error(err.message);
    ...
    try {
        ...
        logger_1.logger.info(`Environment variables updated`);
    }
    catch (err) {
        handleError(err);
    }
...
exports.describe = 'Takes a .env file and uploads all environment variables to a given environment';

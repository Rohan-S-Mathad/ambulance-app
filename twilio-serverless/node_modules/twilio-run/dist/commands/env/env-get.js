...
const debug = (0, logger_1.getDebugFunction)('twilio-run:env:get');
...
async function handler(flags, externalCliOptions) {
    ...
    let config;
    try {
        config = await (0, env_get_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
        logger_1.logger.error(err.message);
    ...
    try {
        ...
        if (!resultVariable) {
            throw new Error(`Could not find environment variable with name ${flags.key} for service "${result.serviceSid}" and environment "${result.environmentSid}".`);
        }
        else {
            (0, output_1.writeOutput)(resultVariable.value);
        }
    }
    catch (err) {
        handleError(err);
...
exports.command = ['get'];
exports.describe = 'Retrieves the value of a specific environment variable';

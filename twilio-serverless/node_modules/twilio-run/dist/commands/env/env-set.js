...
const env_set_1 = require("../../config/env/env-set");
...
async function handler(flags, externalCliOptions) {
    ...
    let config;
    try {
        config = await (0, env_set_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
        logger_1.logger.error(err.message);
    ...
    try {
        ...
        logger_1.logger.info(`${flags.key} has been set`);
    }
    catch (err) {
        handleError(err);
    }
...
exports.command = ['set'];
exports.describe = 'Sets an environment variable with a given key and value';

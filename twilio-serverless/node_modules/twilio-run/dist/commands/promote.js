...
async function handler(flags, externalCliOptions) {
    ...
    const continueWork = await (0, legacy_config_1.default)(flags.cwd);
    if (!continueWork) {
        process.exit(1);
    }
    let config;
    try {
        config = await (0, promote_1.getConfigFromFlags)(flags, externalCliOptions);
    }
    catch (err) {
        debug(err);
    ...
    try {
        ...
        (0, activate_1.printActivateResult)(result, config.outputFormat, config.region);
    }
    catch (err) {
